plots.video.dyads <- function(vms.data,dyads,year,gear,new.variables,dossier.stats.outputs,dossier.data.outputs,n=5,text.dyads=TRUE,
  ...
  action <- sapply(1:(dim(echantillon)[1]),function(x){
    ...
    if (video.par$type == 'series' || video.par$type == 'all'){
      ...
      variable <- "LONGITUDE"
      pdf(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 paste(unlist(strsplit(as.character(date(fecha.min)),"-")),collapse = ""),
                 '-',
                 paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                 '-',variable,'.pdf'),width=10,height=7,pointsize=20)
      par(las=2)
      subplot.cor(tr1,tr2,variable)
      dev.off()
      # ><>><>><>><>
      variable <- "LONGITUDE"
      png(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 paste(unlist(strsplit(as.character(date(fecha.min)),"-")),collapse = ""),
                 '-',
                 paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                 '-',variable,'.png'),width=10,height=7, units = "in", res = 120)#,pointsize=20)
      par(las=2)
      subplot.cor(tr1,tr2,variable)
      dev.off()
      ...
      png(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 '-',
                 paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                 '-',variable,'.png'),width=10,height=7, units = "in", res = 120)
      par(las=2)
      subplot.cor.gif(tr1,tr2,variable)
      dev.off()
      
      #gif ......
      variable <- "LONGITUDE"
      file.name.gif =  paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                              paste(unlist(strsplit(as.character(date(fecha.min)),"-")),collapse = ""),
                              '-',
                              paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),'-',variable,'.gif')
      subplot.cor.gif(file.name.gif, tr1, tr2, variable, r.cal = TRUE)
      #><>><>><>><>
      print("a")
      variable <- "LATITUDE"
      pdf(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
      ...
      for (prox in n.prox:1){
        points(fechas[which(DO <= param$Prox$delta[prox])],DO[which(DO <= param$Prox$delta[prox])],col=colores[prox],cex=1,pch=21,bg=colores[prox])
        texte <- c(texte,paste0("<",param$Prox$delta[prox], " (",round(sum(DO <= param$Prox$delta[prox])/length(fechas),2),")"))
      }
      legend("topleft",texte[n.prox:1],col=colores,pch=20,bty="n")
      dev.off()
      
      #subplot.uni.gif(file.name = "DO.gif", fecha = fechas, variable = DO, nombre = "variable", param_Prox_delta = param$Prox$delta)
      #><>><>><>><> 
      png(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 paste(unlist(strsplit(as.character(date(fecha.min)),"-")),collapse = ""),
                 '-',
      ...
      # genera gif "DISTANCE - Prox"
      colores=brewer.pal(n.prox, "Set1")
      file.name = file.name.gif
      fechas = fechas
      variable = DO
      color = "black"
      cal = FALSE
      nombre = "DISTANCE - Prox"
      param_Prox_delta = param$Prox$delta
      rango = range(DO)
      subplot.uni.DO.gif() # genera gif "DISTANCE - Prox"
      #><>><>><>><>
      ...
      pdf(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 '-',
                 paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                 '-',str_replace_all(variable, "[[:punct:]]", ""),'.pdf'),width=10,height=7,pointsize=20)
      par(las=2)
      subplot.cor(tr1,tr2,variable)
      dev.off()
      ...
      png(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 '-',
                 paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                 '-',str_replace_all(variable, "[[:punct:]]", ""),'.png'),width=10,height=7, units = "in", res = 120)#,pointsize=20)
      par(las=2)
      subplot.cor(tr1,tr2,variable)
      dev.off()
      ...
      #gif ......
      #variable <- "avg.speed"
      file.name =  paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                              paste(unlist(strsplit(as.character(date(fecha.min)),"-")),collapse = ""),
                              '-',
                              paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                              '-',str_replace_all(variable, "[[:punct:]]", ""),'.gif')
      subplot.cor.gif(file.name, tr1, tr2, variable, r.cal = TRUE)
      #><>><>><>><>
      ...
      pdf(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 '.pdf'),width=10,height=7,pointsize=20)
      par(las=2)
      subplot.uni(fecha=fechas,variable=DI0,nombre="DI",cal=TRUE)
      dev.off()
      ...
      png(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 paste(unlist(strsplit(as.character(date(fecha.max)),"-")),collapse = ""),
                 '-',str_replace_all(variable, "[[:punct:]]", ""),
                 '.png'),width=10,height=7, units = "in", res = 120)#,pointsize=20)
      par(las=2)
      subplot.uni(fecha=fechas,variable=DI0,nombre="DI",cal=TRUE)
      dev.off()
      ...
      pdf(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 '.pdf'),width=10,height=7,pointsize=20)
      par(las=2)
      subplot.uni(fecha=fechas,variable=DId0.d,nombre="DId",cal=TRUE)
      dev.off()
      ...
      pdf(paste0(dossier.stats.outputs,gear,'/VariableSeries-id-',gsub(" ", "-", bato1),'-',gsub(" ", "-", bato2),"-",
                 ...
                 '.pdf'),width=10,height=7,pointsize=20)
      par(las=2)
      subplot.uni(fecha=fechas,variable=DItheta,nombre="DItheta",cal=TRUE)
      dev.off()
      ...
      subplot.uni.DI.theta(file.name, fecha=fechas,variable=DItheta,nombre="DItheta",cal=TRUE)
